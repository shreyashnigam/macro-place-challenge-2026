...
class ValidityFirstPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        selector_data = _build_surrogate_data(benchmark) if self.optimize and self.steps > 0 else None
        return _select_by_fast_score(candidates, benchmark, selector_data, eps=self.selector_eps)
            ...
            for start_name, start in starts:
                for cfg_name, cfg in configs:
                    if opt_added >= self.max_candidates:
                        break
                    candidate = _optimize_global(start, benchmark, data, steps=self.steps, cfg=cfg)
                    candidate = _legalize_hard(candidate, benchmark, gap=self.gap, max_rounds=300)
...
def _build_surrogate_data(benchmark: Benchmark) -> dict[str, torch.Tensor]:
    ...
    route_hh_i, route_hh_j, route_hh_w = _limit_weighted_edges(
        hh_i_t, hh_j_t, hh_w_t, route_limit
    )
    route_anchor_i, route_anchor_xy, route_anchor_w = _limit_weighted_anchors(
        anchor_i_t, anchor_xy_t, anchor_w_t, route_limit
    )
...
def _limit_weighted_edges(
    src: torch.Tensor, dst: torch.Tensor, weights: torch.Tensor, limit: int
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if limit <= 0 or len(weights) <= limit:
        return src, dst, weights
    _, order = torch.topk(weights, k=min(limit, len(weights)))
    return src[order], dst[order], weights[order]


def _limit_weighted_anchors(
    src: torch.Tensor, xy: torch.Tensor, weights: torch.Tensor, limit: int
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if limit <= 0 or len(weights) <= limit:
        return src, xy, weights
    _, order = torch.topk(weights, k=min(limit, len(weights)))
    return src[order], xy[order], weights[order]
...
def _surrogate_loss(
    ...
    if len(data["hh_i"]) > 0:
        delta = (pos[data["hh_i"]] - pos[data["hh_j"]]) / scale
    ...
    if len(data["anchor_i"]) > 0:
        delta = (pos[data["anchor_i"]] - data["anchor_xy"]) / scale
        dist = torch.sqrt((delta * delta).sum(dim=1) + 1e-6)
...
def _route_pressure_loss(
    ...
    if len(data["route_hh_i"]) > 0:
        src = pos[data["route_hh_i"]]
        dst = pos[data["route_hh_j"]]
        mid = (src + dst) * 0.5
...
def _centroid_targets(
    ...
    if len(data["hh_i"]) > 0:
        ...
        for i, j, w in zip(hh_i, hh_j, hh_w):
            hi = var_to_hard[i]
            hj = var_to_hard[j]
            targets[i] += hard[hj] * w
            targets[j] += hard[hi] * w
    ...
    if len(data["anchor_i"]) > 0:
        ...
        for i, xy, w in zip(anchor_i, anchor_xy, anchor_w):
            targets[i] += xy * w
            weights[i] += w
...
def _select_by_fast_score(
    ...
    *,
    ...
    scored = [
        (_fast_surrogate_score(placement, benchmark, data), _candidate_preference(name), placement)
    ...
    return min(near_best, key=lambda item: (item[1], item[0]))[2]
...
def _fast_surrogate_score(
    ...
    for nodes_t in benchmark.net_nodes:
        ...
        hpwl = float((coords[:, 0].max() - coords[:, 0].min()) + (coords[:, 1].max() - coords[:, 1].min()))
